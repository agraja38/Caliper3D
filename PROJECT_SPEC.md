...
- Network.framework + Bonjour for local discovery/transfer; never Multipeer Connectivity. CryptoKit for integrity/authentication. Model I/O for future mesh import/export. OSLog with private metadata.
...
No accounts, cloud backend, analytics, telemetry, subscriptions or web wrappers. No automatic execution of imported content. Transfer requires explicit peer approval, authenticated encrypted transport, bounded frames/files, safe relative paths, integrity checks, staging and atomic completion, cancellation and disk-space handling. Bonjour discovery alone is not authentication. Never store credentials, signing assets or personal scans in Git. Installer remains an abstraction until safe Xcode/provisioning design is implemented; no insecure sideloading.

## Workflow
Keep all work on main as the single repository branch, per the owner’s updated instruction. Do not create additional branches unless requested. Use logical commits, no force pushes/history rewriting. Keep IMPLEMENTATION_STATUS.md and NEXT_STEPS.md honest and current. Build both targets and test shared packages. Real transfer verification is the current hardware gate; do not begin reconstruction before it passes.
...
Capture datasets use a separate schema-1 capture.json under app-private Application Support/Caliper3D/Captures/<UUID>, with Images and Checkpoints directories. Only actual completed sessions with inspected nonempty image files become ready. Human names do not form paths. Cancel/failure preserves incomplete datasets; deletion is explicit and UUID-scoped. Over-capture is enabled for later Mac reconstruction. Checkpoints do not promise saved-session resume. See docs/ObjectCapture.md; the user verified a real mouse scan through saved review and app relaunch before Session 3.

## Transfer architecture (Session 3)
Caliper3DTransfer owns the version-1 control/binary framing, resource policy, capture transfer manifest and incremental integrity/receiver state logic. Network/TLS, trust storage, staging and UI adapters remain separate boundaries. TLSChannel, Keychain identity/trust and exporter-bound commit/reveal pairing components are implemented; ConnectionCoordinator integrates production Bonjour, explicit pairing/trusted reconnect and streaming UI. PreparedCaptureTransfer obtains indexed source reads through CaptureRepository; IncomingCaptureStore owns staging, integrity and durable rehashed resume. See docs/TransferProtocol.md. The received project and transfer UUIDs equal capture UUID, with manifest digest binding and explicit duplicate/conflict handling. Normal listener mode accepts stored pins only; first-pair mode requires explicit bounded user action.
